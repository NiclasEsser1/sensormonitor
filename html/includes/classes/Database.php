...
class Database {
	...
	public function getCurrentData($nodeId) {
		...
		$subDate 	= $dateObject->substractDate('PT180M');
		...
		$sql = "SELECT MAX(ID) as ID,
			...
			WHERE nodeId = ?
			AND datetime >= ?";
		$data = $this->queryDatabase($sql, array_values($values));
	...
	{
		$sql = "INSERT INTO measurement(nodeId,temperature,humidity,pressure,voltage) VALUES(?,?,?,?,?)";
